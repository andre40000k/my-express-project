...
## 📦 Установка зависимостей (если не используете Docker)
...
Теперь проект поддерживает запуск в контейнерах с использованием Docker. Для этого выполните следующие шаги:
...
Проект подключен к базе данных **MongoDB**. Если вы используете Docker, MongoDB будет запущена в отдельном контейнере. Если запускаете проект без Docker, убедитесь, что у вас запущен сервер MongoDB перед стартом проекта.
